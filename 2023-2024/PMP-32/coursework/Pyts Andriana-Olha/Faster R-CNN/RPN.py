...
class AnchorGenerator(nn.Module):
    # Генерує anchor на зображеннях за допомогою anchor_scale та anchor_aspect_ratio.
    ...
    # Генерує anchor для кожної карти об'єктів на зображеннях.    
    def forward(self, features):
        images_anchors = self.generate_images_anchors(features, self.downsample)
        return images_anchors

    # Створює базові прив'язки, використовуючи масштаби прив'язок і співвідношення сторін.
    ...
    # Генеруйте anchor для всіх зображень на основі карт об'єктів.
...
class RegionProposalNetwork(nn.Module):
    # RPN для створення об'єктних пропозицій.
    ...
    # Присвоює anchors мітки базової інформації та обмежувальні рамки.
    ...
    def convert(self, bbox_cls, bbox_regression):
        #  Перетворення вихідної форми згорткового шару 
    ...
    # Фільтрує пропозиції на основі оцінок об'єктності та застосовуйте не-максимальне придушення (NMS).
